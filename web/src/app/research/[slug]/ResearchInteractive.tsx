...
export default function ResearchInteractive({ slug, availableLocales, currentLocale, showEngagement }: Props) {
  const router = useRouter();
  const [readProgress, setReadProgress] = useState(0);

  useEffect(() => {
    trackContentView(slug, 'research');
  }, [slug]);
  ...
  return (
    <>
      {/* Reading progress bar - cyan for research */}
        ...
        <div className="fixed top-3 right-4 z-[55] flex items-center gap-1 text-sm">
          {availableLocales.map((locale, i) => (
            <React.Fragment key={locale}>
              ...
              <button
                onClick={() => router.push(`/research/${slug}?lang=${locale}`)}
