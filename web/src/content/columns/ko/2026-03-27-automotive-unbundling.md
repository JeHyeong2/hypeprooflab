...
readTime: "30분"
excerpt: "100년간 쌓인 법과 돈의 구조 위에서, 기술은 수평이고 구조는 수직이다. Evans의 번들/언번들 프레임이 자동차에서는 왜 절반만 맞는지, 그리고 AI가 이 구조를 어떻게 바꾸고 있는지."
lang: "ko"
authorType: "creator"
---
...
전동시트 결함으로 [미국·캐나다 69,000대 + 한국 58,000대](https://www.edmunds.com/car-news/hyundai-palisade-kia-telluride-rear-seat-recall.html). [계기판 디스플레이가 꺼지는 소프트웨어 버그](https://www.nhtsa.gov/recalls). [안전벨트 버클 결함 568,000대](https://www.nhtsa.gov/vehicle/2023/HYUNDAI/PALISADE). 한 차종, 세 리콜, 세 개의 완전히 다른 팀.
...
Automotive 업계에서 OEM과 수많은 PoC부터 고도화를 거친 양산 프로세스, 양산 후 확대전개와 OTA까지 경험하면서 — 이 질문은 이 업계에 있는 사람이라면 누구나 한다. 하지만 "그래서 누구 책임이냐"는 서로가 서로에게 차마 말할 수 없는 답이다. 우리가 만드는 소프트웨어는 부품 경계를 넘어 차량 전체의 데이터를 다룬다. 하지만 우리가 들어가야 하는 서플라이 체인은 부품 단위로 잘려 있다. **기술은 수평이고, 구조는 수직이다.**

한 가지 미리 밝혀둔다. 나는 이 구조 안에서 일하는 사람이고, 이 글은 그 포지션에서 바라본 분석이다. "silo를 연결하는 자가 가치를 가져간다"는 결론이 내가 속한 위치에 유리한 분석일 수 있다는 점은 인지하고 있다.
...
한국에서는 [비팩토리](https://www.bfactory.co) 대표 [노정석](https://byline.network/2025/08/2-ai/)(연쇄창업가 — 태터앤컴퍼니를 구글에 매각한 한국 최초의 구글 인수 사례)이 그의 팟캐스트 [AI Frontier EP91(2026.3.21)](https://aifrontier.kr/ko/episodes/ep91/)에서 이 프레임을 AI 시대의 비즈니스에 적용했다. 두 가지 핵심 테제:
...
## 3. Tier 구조는 어떻게, 왜 이렇게 됐는가

자동차 서플라이 체인은 겉보기에 단순하다:

**Tier-3 (소재)** → **Tier-2 (부품)** → **Tier-1 (시스템)** → **OEM (브랜드)**

이 구조는 처음부터 이랬던 게 아니다. 100년에 걸쳐, 여섯 가지 힘이 이 형태를 만들었다.

### 수직통합의 실패에서 시작됐다

Henry Ford의 [River Rouge 공장(1928)](https://www.thehenryford.org/visit/ford-rouge-factory-tour/)은 수직통합의 극단이었다. 2,000에이커 부지에서 철광석, 고무, 석탄이 들어가 41시간 만에 완성차가 나왔다. Ford는 켄터키 탄광, 미시간 철광, 브라질 고무 농장(Fordlandia), 선박, 철도까지 소유했다. 모든 걸 직접 만들면 원가를 통제할 수 있다는 논리였다.

Alfred Sloan의 GM은 정반대 길을 택했다. 전문화된 외부 업체가 규모의 경제로 더 싸게, 더 잘 만들 수 있다고 봤다. 그리고 [대공황이 Sloan의 손을 들어줬다](https://www.cambridge.org/core/journals/journal-of-economic-history/article/abs/explaining-vertical-integration-lessons-from-the-american-automobile-industry/9F45384CE2B021DFCB68579C03B9DC3E). Ford의 거대한 고정비 구조는 수요 급감에 대응할 수 없었고, GM의 외주 기반 유연성이 생존을 결정했다. 이후 전체 산업이 서플라이어 전문화 방향으로 기울었다.

### 도요타가 계층을 고착화했다

1950-80년대, 서구 OEM들이 부품을 아웃소싱하는 동안 일본에서는 다른 구조가 만들어졌다. 전후 재벌(자이바츠) 해체 이후 형성된 [케이레츠(系列)](https://en.wikipedia.org/wiki/Keiretsu) — 교차 지분, 장기 거래 관계, 주거래 은행 중심의 기업 네트워크 — 가 계층화된 서플라이 피라미드의 원형을 제공했다.

[오노 다이이치(大野耐一)](https://en.wikipedia.org/wiki/Taiichi_Ohno)와 도요다 에이지가 1948-1975년 사이에 개발한 Toyota Production System(TPS)은 이 장기 관계 위에서 Just-in-Time을 실현했다. MIT의 [International Motor Vehicle Program(IMVP)](https://dspace.mit.edu/handle/1721.1/1782)이 14개국, $500만, 5년에 걸친 연구를 통해 이 시스템의 우위를 입증했고, Womack, Jones, Roos의 *[The Machine That Changed the World](https://www.lean.org/store/book/the-machine-that-changed-the-world/)(1990)*로 출판되면서 "lean production"이라는 개념이 전 세계로 퍼졌다. 서구 OEM들도 Tier-1/Tier-2 계층 구조를 Toyota 모델에 맞춰 재편했다.

### 법이 계층에 책임을 부착했다

구조가 만들어진 다음, 법이 그 구조에 책임을 고정시켰다:

- **[MacPherson v. Buick Motor Co. (1916)](https://law.justia.com/cases/new-york/court-of-appeals/1916/217-n-y-382-1916.html)** — 소비자가 직접 구매하지 않은 제조사도 소송할 수 있게 됐다. OEM이 전체 서플라이 체인의 품질을 관리해야 할 법적 근거가 생겼다.
- **[Greenman v. Yuba Power Products (1963)](https://supreme.justia.com/cases/california/1963/)** — 미국에 strict product liability(무과실 책임)를 확립했다. 40년 안에 거의 모든 주가 채택했다.
- **[EU Product Liability Directive 85/374/EEC (1985)](https://eur-lex.europa.eu/legal-content/EN/TXT/?uri=celex:31985L0374)** — 유럽 전역에 무과실 책임을 도입했다. 결정적으로: 생산자를 특정할 수 없는 경우, 서플라이 체인의 각 공급자가 생산자로 취급된다.

이 법적 진화가 Tier 구조에 책임의 폭포(liability cascade)를 만들었다. OEM은 차량 브랜드 = 피고인. 이 리스크를 관리하기 위해 Tier-1과 계약적 면책(indemnification) 체인을 형성하고, Tier-1은 다시 Tier-2와 같은 구조를 반복한다. **Tier 번호는 기술적 깊이가 아니라, 책임의 거리(distance from liability)를 나타낸다.** 조직도가 BOM(Bill of Materials)을 따라가고, Type Approval(형식 승인)이 같은 구조를 강화한다 — UNECE R13H(제동), R48(등화), R155(사이버보안), 규제 하나당 담당 팀 하나.

### 규제와 품질 표준이 구조를 감사 체계로 굳혔다

[UNECE WP.29](https://unece.org/wp29-introduction)(차량 규제 조화 세계포럼)의 형식 승인 체계에서 인증서 보유자는 OEM이지만, 적합성 증명은 서플라이 체인을 따라 흘러내린다. 품질 표준도 같은 방향이었다. Big Three가 1994년 만든 [QS-9000](https://www.omnex.com/iatf-16949-automotive-qms-history)에서 시작해, 미국·독일·프랑스·이탈리아 표준을 통합한 ISO/TS 16949(1999)을 거쳐 현재의 IATF 16949(2016)에 이르렀다. 각 표준은 공급자 관리를 의무화해서, Tier 계층 구조를 감사 가능한 프로세스로 제도화했다.

### 자본 효율이 이 구조를 합리적으로 만들었다

OEM이 스탬핑, 전자장비, 유리, 고무, 인테리어 트림 모두에서 world-class일 수는 없다. 전문 서플라이어는 복수의 OEM을 동시에 서빙하면서 규모의 경제를 달성한다. Bosch는 전 세계 거의 모든 OEM에 브레이크 시스템을 공급하고, Continental은 수십 개 OEM에 ECU를 납품한다. 하나의 OEM이 혼자서 이 규모를 달성하는 건 불가능하다.

**요약하면**: 수직통합의 실패(Ford) → 전문화의 승리(Sloan/Toyota) → 법적 책임의 부착(PLD) → 규제/표준의 제도화(UNECE, IATF) → 자본 효율의 합리화. 이 다섯 단계가 겹겹이 쌓여서, 오늘의 Tier 구조를 만들었다.

### 테슬라는 왜 이 구조를 무시할 수 있었는가

테슬라는 [서플라이 체인의 약 80%를 내재화](https://supplychain360.io/teslas-vertical-integration-revolutionizes-supply-chains/)한다. 배터리 셀(4680, 건식 전극 공정), 컴퓨트 칩(FSD HW4, AI6), 파워트레인, 소프트웨어 풀스택, 충전 인프라, 심지어 보험까지. OEM이자 Tier-1이자 서비스 사업자다.

가능했던 이유는 **레거시가 없었기 때문이다.** 기존 Tier-1과의 5-7년 장기 계약이 없었다. 해고에 저항하는 노조(UAW, IG Metall)가 없었다. ICE 파워트레인에 묶인 설비 투자가 없었다. 빈 종이에서 시작했기 때문에, 100년간 쌓인 구조적 관성을 무시하고 수직통합을 선택할 수 있었다.

하지만 수직통합에도 대가가 있다. 2025년 8월, 테슬라는 4년간 자체 개발한 AI 훈련 슈퍼컴퓨터 [Dojo 프로젝트를 폐기](https://digitalhabitats.global/blogs/tesla-1/exclusive-tesla-kills-dojo-for-ai6-here-s-why)했다 — 커스텀 칩(D1), 커스텀 훈련 타일까지 만들었지만, 최첨단 반도체를 자체 개발하는 건 테슬라에게도 자본 집약적이었다. 유리, 타이어, 반도체 파운드리는 여전히 외부에 의존한다.

전통 OEM이 테슬라를 따라할 수 없는 이유는 기술력 부족이 아니라 **구조적 잠금(structural lock-in)**이다. 수십 년간의 Tier-1 계약, NRE 매몰비용, 노조 협약, ICE와 EV의 이중 투자, 15-20년 차량 수명 동안의 애프터마켓 의무 — 이 모든 것이 겹겹이 쌓여 있다. **그래서 대부분의 OEM에게 현실적 선택은 수직통합이 아니라, 기존 Tier 구조 안에서 SW 레이어를 재설계하는 것이다.**
...
VW의 CARIAD가 [누적 운영손실만 €75억에 달하는 교훈](https://insideevs.com/news/753673/vw-group-cariad-billions-losses-2024/)이다. 내부 이전가격(transfer pricing) 합의가 안 됐다. VW, Audi, Porsche, Škoda 각 브랜드가 "왜 우리가 다른 브랜드 SW 비용을 부담하느냐"고 물었고, 답은 끝내 나오지 않았다. CEO는 2회 교체됐고, 전략 차량 Trinity는 2년 이상 지연됐다. VW는 결국 [Rivian에 $50억을 투자](https://www.volkswagen-group.com/en/press-releases/faster-leaner-more-efficient-rivian-and-volkswagen-group-announce-the-launch-of-their-joint-venture-18828)해서 외부에서 사왔다 — 내부 구축 5년의 실패를 인수로 우회한 것이다. 모든 멀티브랜드 OEM 그룹 — 현대차그룹, Toyota, Stellantis(14개 브랜드) — 이 동일한 구조적 질문에 직면한다.

### 기둥 2: 워런티 — 변화 자체가 재무 리스크

[Ford의 연간 워런티 비용은 약 $48억](https://www.sec.gov/cgi-bin/browse-edgar?action=getcompany&CIK=0000037996&type=10-K)(2023 10-K 기준), 대당 $2,800이다. 현대기아는 2023년 Theta II 엔진 리콜로 [$39억을 일시 충당](https://dart.fss.or.kr/)했다. 워런티는 OEM 재무제표에서 가장 예측 불가능한 항목이며, 한 번의 대규모 리콜이 분기 실적을 뒤집을 수 있다.
...
여기서 반론을 하나 짚어야 한다. **Tesla와 중국 OEM은 silo를 해체할 필요가 없었다.** Tesla는 처음부터 수직 통합 — 자체 칩(FSD Computer), 자체 소프트웨어, 자체 OTA, fleet 데이터 루프를 이미 닫았다. BYD는 배터리에서 반도체까지 수직 통합을 완성했고, Xiaomi는 스마트폰 생태계를 그대로 차량에 이식했다. 이들에게 silo는 "해체해야 할 구조"가 아니라 "처음부터 없는 구조"다.

이건 이 글의 프레임이 가진 한계를 명확히 한다. **이 분석은 기존 silo를 가진 전통 OEM — Hyundai, VW, Toyota, Stellantis — 의 전환 과정에 대한 것이다.** Tesla/BYD처럼 처음부터 다른 번들로 시작한 기업에는 Evans의 unbundle/re-bundle이 적용되지 않는다. 그들은 "해체 후 재결합"이 아니라 "처음부터 다른 결합"이기 때문이다. 오히려 전통 OEM에게 진짜 위협은 silo 해체의 난이도가 아니라, **silo 없이 태어난 경쟁자가 이미 닫힌 루프를 돌리고 있다**는 사실이다.
...
### Frontier Lab들의 진입 — 법적 리스크 없는 가치 추출

6장에서 Frontier Model이 분석 레이어를 commodity화한다고 분석했다. 여기서는 그 구체적 양상을 본다. [OpenAI + Applied Intuition](https://www.appliedintuition.com/news/applied-intuition-openai), [Google + Waymo/Android Automotive](https://waymo.com/blog/2026/02/the-waymo-world-model-a-new-frontier-for-autonomous-driving-simulation/), [Microsoft Azure SDV](https://www.microsoft.com/en-us/industry/blog/manufacturing-and-mobility/2026/01/07/ces-2026-powering-the-next-frontier-in-automotive/) — 이들의 공통점은 **Tier에 직접 안 들어간다**는 것이다. 제조물책임을 지지 않는 클라우드/API 레이어에서 가치를 추출한다. 3장에서 논의한 "법적 리스크 설계"의 가장 세련된 형태다.
...
6장에서 다뤘듯이, "non-verifiable을 verifiable로 전환할 수 있는 환경"을 자동차에 적용하면: **부품 silo 사이를 관통하는 데이터 루프를 소유하는 것**이다. 차량에서 데이터를 수집하고, 진단하고, 예측 모델로 만들어 다시 배포하는 루프. 이 루프의 한 조각을 교체하면 전체가 깨진다.
...
하나만 예측하겠다. **2030년까지 전통 OEM 중 최소 하나가 cross-ECU 소프트웨어 책임을 계약적으로 인수하는 새로운 Tier-0.5 모델을 만들 것이다.** 그 OEM이 처음으로 "연결의 책임"을 설계하는 순간, 나머지 OEM의 silo는 5년 안에 재편된다. 그리고 그 계약 모델을 만드는 데 필요한 건 AI 기술이 아니라, 제조물책임법 해석과 재보험 구조에 대한 이해다. 이게 맞는지는 5년 후에 안다.
...
| 8 | VW CARIAD — 누적 운영손실 €75억 | https://insideevs.com/news/753673/vw-group-cariad-billions-losses-2024/ |
...
| 12 | 계기판 SW 리콜 — Cars.com | https://www.nhtsa.gov/recalls |
| 13 | 팰리세이드 전체 리콜 이력 — Cars.com | https://www.nhtsa.gov/vehicle/2023/HYUNDAI/PALISADE |
...
| 25 | Ford Motor Company 10-K SEC Filing (워런티 비용) | https://www.sec.gov/cgi-bin/browse-edgar?action=getcompany&CIK=0000037996&type=10-K |
| 26 | VW Group + Rivian JV 공식 발표 | https://www.volkswagen-group.com/en/press-releases/faster-leaner-more-efficient-rivian-and-volkswagen-group-announce-the-launch-of-their-joint-venture-18828 |
| 27 | 현대기아 분기 보고서 (DART) | https://dart.fss.or.kr/ |
...
| 33 | UNECE WP.29 — 차량 규제 조화 세계포럼 | https://unece.org/wp29-introduction |
