...
여기서 중요한 설정이 하나 더 있다. Message Content Intent를 반드시 활성화해야 한다. Discord는 2022년부터 봇의 메시지 내용 접근에 별도 권한을 요구한다([Discord Gateway Intents](https://discord.com/developers/docs/events/gateway#gateway-intents)). Developer Portal의 Bot 설정에서 "Message Content Intent" 토글을 켜지 않으면, 에이전트가 메시지를 받아도 내용이 빈 문자열로 들어온다. 이걸 모르면 "왜 봇이 반응을 안 하지?"라며 한참을 헤매게 된다.
...
이 글을 읽고 직접 시도해보고 싶다면, [OpenClaw의 공식 문서](https://docs.openclaw.ai)에서 시작하는 것을 추천한다. 그리고 하나만 기억하자 — 처음엔 누구나 삽질한다. 그 삽질이 쌓여서 나만의 완벽한 AI 비서가 만들어지는 것이다.
...
| # | 출처 | URL |
|---|------|-----|
| 1 | [OpenClaw — Personal AI Assistant](https://openclaw.ai) | OpenClaw 공식 홈페이지 |
| 2 | [OpenClaw Documentation](https://docs.openclaw.ai) | 설치, 설정, 스킬 가이드 공식 문서 |
| 3 | [Discord Developer Portal](https://discord.com/developers/docs/intro) | Discord 봇 생성 및 토큰 발급 가이드 |
| 4 | [Discord Gateway Intents](https://discord.com/developers/docs/events/gateway#gateway-intents) | Message Content Intent 등 봇 권한 설정 |
| 5 | [Node.js ES Modules Documentation](https://nodejs.org/api/esm.html) | Node.js 18+ ES Module 지원 문서 |
| 6 | [Gartner — AI Agent Forecast (2026)](https://www.gartner.com/en/articles/intelligent-agent-in-ai) | 기업 40%가 AI 에이전트 기반 자동화 도입 전망 |
| 7 | [OWASP Top Ten](https://owasp.org/www-project-top-ten/) | 웹 애플리케이션 보안 원칙 |
