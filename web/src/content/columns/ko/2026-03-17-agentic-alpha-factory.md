...
readTime: "8분"
excerpt: "\"하루 만에 성공\"이라고? 7개월째 개밥먹기 중인 사람의 솔직한 기록. 에이전틱 퀀트 하니스의 3가지 병목과 Pod Shop을 AI로 복제하려는 비전."
---

*"하루 만에 성공"이라고? 7개월째 개밥먹기 중인 사람의 솔직한 기록*
...
7개월 전, 채팅으로 아이디어를 내면 전략 리서치 → 백테스트(과거 데이터로 전략이 실제로 돈을 벌었을지 시뮬레이션하는 것) → 자가 개선(AI가 결과를 보고 스스로 전략을 고치는 것)까지 이어지는 에이전틱 퀀트 하니스, **[VibeTrading](https://www.linkedin.com/posts/jiwoong-kim-b9934417a_introducing-the-prototype-of-my-new-personal-activity-7361167113148878848-quDk/)**의 시연 장면을 공개했다.

당시에는 Agent가 리서치 결과를 코드로 변환하는 데 병목이 있을 거라 생각했다. 그래서 AI가 복잡한 프로그래밍 언어 대신 간단한 수식만 쓰면 되도록, 전용 축약 언어(DSL)를 만들어 해결했다. "최근 20일간 종가의 순위를 매겨라"를 `ts_rank(close, 20)` 한 줄로 표현하는 식이다. 이렇게 AI의 부담을 줄이는 데 많은 시간을 썼다.
...
AI가 코드를 아무리 잘 짜도, "거래정지된 종목의 가격 데이터가 비어있을 때 그게 정말 비어있는 건지 아니면 데이터 벤더의 오류인지"를 판단하려면 과거 데이터 시뮬레이션 도구가 도는 구조를 알아야 하고, 이걸 처리할 때 '당시에는 몰랐을 미래 정보를 실수로 쓰고 있진 않은가'(look-ahead bias) 하는 고민 등은 도메인 지식이다. 여기는 에이전트가 대신해줄 수 없다.
...
| # | 출처 | URL |
|---|------|-----|
| 1 | 원본 글 — LinkedIn | https://www.linkedin.com/feed/update/urn:li:activity:7438980199616184321/ |
| 2 | VibeTrading 시연 — Agentic Quant Research Tool | https://www.linkedin.com/posts/jiwoong-kim-b9934417a_introducing-the-prototype-of-my-new-personal-activity-7361167113148878848-quDk/ |
| 3 | Claude Code — Anthropic 공식 문서 | https://docs.anthropic.com/en/docs/claude-code |
| 4 | Building Effective Agents (Ralph Loop) — Anthropic Research | https://www.anthropic.com/research/building-effective-agents |
| 5 | OMC / OMX 커뮤니티 | https://discord.com/invite/JkT8CT5gq |
