...
AI가 코드를 아무리 잘 짜도, "거래정지된 종목의 가격 데이터가 비어있을 때 그게 정말 비어있는 건지 아니면 데이터 벤더의 오류인지"를 판단하려면 백테스팅 도구가 도는 구조를 알아야 하고, 이걸 처리할 때 미래 참조(look-ahead bias)가 없을까 하는 고민 등은 도메인 지식이다. 여기는 에이전트가 대신해줄 수 없다.
