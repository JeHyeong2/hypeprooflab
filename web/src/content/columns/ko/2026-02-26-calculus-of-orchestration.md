...
물론 너무 잦은 개입은 에이전트의 자율성을 죽인다. [나이퀴스트 정리](https://en.wikipedia.org/wiki/Nyquist%E2%80%93Shannon_sampling_theorem)가 신호처리에서 말하듯, 적절한 샘플링 주파수가 있다. 너무 낮으면 정보를 놓치고, 너무 높으면 노이즈만 잡는다.
...
미적분의 언어로 말하면, 이건 이미 정의된 함수 위에서 기울기를 따라가는 것이다. [경사하강법](https://en.wikipedia.org/wiki/Gradient_descent)처럼. 어딘가에 최저점이 있고, 우리는 기울기를 읽으며 그 최저점으로 수렴한다.
...
[제어공학](https://en.wikipedia.org/wiki/Control_theory)에서 이 구조는 익숙하다. 피드백 제어 루프. 플랜트(AI 에이전트)가 출력을 내고, 센서(인간의 판단)가 출력을 측정하고, 컨트롤러(인간의 피드백)가 오차 신호를 생성해 플랜트에 다시 입력한다.
...
| # | 출처 | URL |
|---|------|-----|
| 1 | [Nyquist–Shannon Sampling Theorem — Wikipedia](https://en.wikipedia.org/wiki/Nyquist%E2%80%93Shannon_sampling_theorem) | 적절한 샘플링 주파수에 대한 신호처리 이론 |
| 2 | [Human-in-the-loop — Wikipedia](https://en.wikipedia.org/wiki/Human-in-the-loop) | 인간 피드백 루프 기반 시스템 설계 개념 |
| 3 | [Gradient Descent — Wikipedia](https://en.wikipedia.org/wiki/Gradient_descent) | 경사하강법 — 기울기를 따라 최적점으로 수렴하는 최적화 기법 |
| 4 | [Control Theory — Wikipedia](https://en.wikipedia.org/wiki/Control_theory) | 피드백 제어 루프와 동역학 시스템 이론 |
