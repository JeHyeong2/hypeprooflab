#!/usr/bin/env zsh
# notify-discord.sh — Send DM to Jay via HypeproofClaude Discord bot
# Usage: notify-discord.sh "message text"
#   or:  echo "message" | notify-discord.sh
#
...
# Create DM channel
DM_CHANNEL=$(curl -sf -X POST "https://discord.com/api/v10/users/@me/channels" \
...
curl -sf -X POST "https://discord.com/api/v10/channels/$DM_CHANNEL/messages" \
  -H "Authorization: Bot $DISCORD_TOKEN" \
  -H "Content-Type: application/json" \
  -d "$PAYLOAD" >/dev/null 2>&1 || {
  echo "WARN: failed to send Discord DM" >&2
  exit 0
}
