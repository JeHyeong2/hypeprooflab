#!/usr/bin/env python3
"""
Google Slides Generator v11 — Grid alignment system
All coordinates use grid constants (MARGIN_L, col2, col3, col4, etc.)
Font sizes use named constants (FONT_TITLE, FONT_BODY, etc.)
"""
import sys, os
...
# === Grid Constants (all slides MUST use these) ===
MARGIN_L = emu(0.5)
MARGIN_R = emu(0.3)
MARGIN_T = emu(0.95)
CONTENT_W = emu(9.0)
...
def col2(i): return MARGIN_L + i * (COL2_W + COL2_GAP)
def col3(i): return MARGIN_L + i * (COL3_W + COL3_GAP)
def col4(i): return MARGIN_L + i * (COL4_W + COL4_GAP)
...
_c = 0
def uid():
    global _c; _c += 1; return f'o{_c:05d}'

def ts(sz, color=PRIMARY, bold=False, italic=False):
    s = {'fontSize': {'magnitude': sz, 'unit': 'PT'}, 'foregroundColor': {'opaqueColor': rgb(color)}, 'bold': bold, 'fontFamily': FONT}
...
def _tx(oid, t): return {'insertText': {'objectId': oid, 'text': t, 'insertionIndex': 0}}

def _st(oid, style, s=0, e=None):
    r = {'updateTextStyle': {'objectId': oid, 'style': style, 'fields': 'fontSize,foregroundColor,bold,fontFamily,italic'}}
    r['updateTextStyle']['textRange'] = {'type': 'FIXED_RANGE', 'startIndex': s, 'endIndex': e} if e else {'type': 'ALL'}
    return r

def _pa(oid, a='START', ls=100):
    style = {'alignment': a}; f = ['alignment']
    if ls != 100: style['lineSpacing'] = ls; f.append('lineSpacing')
    return {'updateParagraphStyle': {'objectId': oid, 'style': style, 'fields': ','.join(f), 'textRange': {'type': 'ALL'}}}

def _fi(oid, c):
    return {'updateShapeProperties': {'objectId': oid,
        'shapeProperties': {'shapeBackgroundFill': {'solidFill': {'color': rgb(c)}}, 'outline': {'propertyState': 'NOT_RENDERED'}},
        'fields': 'shapeBackgroundFill,outline'}}

def _bg(pid, c):
    return {'updatePageProperties': {'objectId': pid, 'pageProperties': {'pageBackgroundFill': {'solidFill': {'color': rgb(c)}}}, 'fields': 'pageBackgroundFill'}}

def rect(pid, x, y, w, h, c):
    o = uid(); return [_sh(pid, o, x, y, w, h, 'RECTANGLE'), _fi(o, c)]

def rrect(pid, x, y, w, h, c):
    o = uid(); return [_sh(pid, o, x, y, w, h, 'ROUND_RECTANGLE'), _fi(o, c)]

def tb(pid, x, y, w, h, t, sz, c=PRIMARY, b=False, a='START', it=False, ls=100):
    o = uid(); return [_sh(pid, o, x, y, w, h), _tx(o, t), _st(o, ts(sz, c, b, it)), _pa(o, a, ls)]

def card(pid, x, y, w, h, c=WHITE):
    r = rrect(pid, x + emu(0.02), y + emu(0.02), w, h, SHADOW)
    r += rrect(pid, x, y, w, h, c)
    return r

def section_header(pid, num, title, subtitle=None):
    """Consistent section header: number badge + conclusion title."""
    r = []
    r += rrect(pid, MARGIN_L, emu(0.28), emu(0.55), emu(0.35), ACCENT)
    r += tb(pid, MARGIN_L, emu(0.30), emu(0.55), emu(0.3), num, 14, WHITE, True, 'CENTER')
...
def s01_title(pid, s):
    ...
    # Korean program name — primary entry point for Korean-speaking audience
    ...
    # Right side workshop photo (1.72:1 ratio → 3.3 x 1.92)
    img_id = uid()
...
def s04_concept(pid, s):
    ...
    # Three-part diagram: Child → AI ← Parent using col3 grid
    # Left: Child card
    child_x = col3(0)
...
def s05_curriculum(pid, s):
    ...
    # Variable widths: Phase 3 (90min core) gets 30% more width — flush with CONTENT_W
    ...
    # Bottom callout: 10-second feedback loop
...
def s06_partnership(pid, s):
    ...
    # Bottom: competitive context bar
...
def s08_faculty(pid, s):
    ...
    for i, m in enumerate(members):
        ...
        # Profile photo (1.0×1.0, centered in card)
        photo_w = emu(1.0)
...
def s09_closing(pid, s):
    ...
    # Workshop banner — full width, aspect ratio 5.46:1
    banner_w = CONTENT_W
...
BUILDERS = {'01': s01_title, '02': s02_market, '03': s03_problem, '04': s04_concept,
            '05': s05_curriculum, '06': s06_partnership, '07': s07_roadmap, '08': s08_faculty, '09': s09_closing}

def generate(title="Future AI Leader's Academy — 동아일보 제안 v11"):
    global _c; _c = 0
    ...
    for i in range(len(SLIDES)):
        p = f'page_{i:02d}'; pages.append(p)
    ...
    for i, s in enumerate(SLIDES):
        ...
        if b: reqs.extend(b(pages[i], s)); print(f'  Slide {i+1}: {s["build"]}')

    for start in range(0, len(reqs), 400):
        svc.presentations().batchUpdate(presentationId=pid, body={'requests': reqs[start:start+400]}).execute()

    url = f'https://docs.google.com/presentation/d/{pid}/edit'
    print(f'🎉 {len(reqs)} reqs → {url}')
    return url
